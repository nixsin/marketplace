...
describe('committed GraphQL schema', () => {
  ...
  it.each([
    ...
    ['Inquiry', 'delivered: Boolean!'],
    ['Product', 'canReceiveInquiries: Boolean!'],
  ])('%s exposes %s', (_type, field) => {
    expect(schema).toContain(field);
  });

  it('never exposes the seller WhatsApp number', () => {
    // The send is server-side precisely so a scraper cannot harvest seller
    // numbers; a field slipping into the schema would undo that silently.
