 ...
 * variable: that export THROWS when unset, which is right for apps/web (a
 * localhost canonical URL reaches crawlers) and wrong here. This service
 * already handles an absent value better than a throw would -- it omits the
 ...
 * boot the whole API would trade a degraded message for no message at all.
 *
 * That is degradation, not a silent fallback: the old
 * `SITE_URL = ... ?? "http://localhost:3000"` default meant production
 * resolved a localhost link and SENT it, which is the failure this replaced.
 * The contract in @medinstru/config marks NEXT_PUBLIC_SITE_URL required on
 * render, so a real deploy still refuses to start without it -- the hard
 * failure lives at boot, where it can be acted on, rather than here.
 */
