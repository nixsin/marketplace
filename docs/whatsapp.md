...
## `SENT` means accepted, not received

`InquiryStatus.SENT` means **the provider accepted the message** — nothing
more. Meta can accept a send and still fail to deliver it: an invalid number,
a blocked contact, a number no longer on WhatsApp.

**None of this reaches the buyer yet.** The GraphQL `Inquiry` type still
exposes no delivery field, and the confirmation still says only that the
inquiry was recorded — which stays true whether the send succeeded or not.
Surfacing the outcome is a separate change, deliberately, because the
confirmation copy is the highest-risk part of this feature by review history:
three separate rounds on the unsplit version were about copy claiming more
than the API knew. When it lands, the wording must not exceed *accepted* —
"on its way to this seller", never "this seller has your question".
