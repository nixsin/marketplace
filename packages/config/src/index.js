...
// as `@medinstru/config/web`, because they THROW when unset and this entry is
// imported by six Node scripts that never read them -- including
// scripts/ai-code-review-precheck.mjs, which runs on every `git push`. An
// eager throw here would have made a missing NEXT_PUBLIC_API_URL block
// pushing, which is a spectacular way to punish an unrelated importer.
//
// Same reasoning that put sourcemap-token behind its own subpath: the main
// entry is shared by the browser and by plain Node tooling, so anything with
// a hard requirement or a Node-only dependency gets its own door.
